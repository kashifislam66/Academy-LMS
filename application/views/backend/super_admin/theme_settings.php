        ...
        <div class="card">
            <div class="card-body">
                <h4 class="page-title"> <i class="mdi mdi-apple-keyboard-command title_icon"></i>
                    ...
                    <a href="https://academy-lms.com/themes" target="_blank"
                        class="btn btn-outline-primary btn-rounded alignToTitle"> <i class="mdi mdi-cart"></i>
                        <?php echo get_phrase('buy_new_theme'); ?></a>
                </h4>
            </div> <!-- end card body-->
        </div> <!-- end card -->
    </div><!-- end col-->
                                    ...
                                    <div class="card-deck">
                                        <div class="card d-block">
                                            <img class="card-img-top"
                                                src="<?php echo base_url('assets/frontend/' . $installed_theme . '/preview.png'); ?>"
                                                alt="Card image cap">
                                            <div class="card-body">
                                                <h5 class="card-title"><?php echo ucfirst($installed_theme); ?></h5>
                                                <div class="">
                                                    <?php if (get_frontend_settings('theme') == $installed_theme) : ?>
                                                    <a href="javascript::" class="btn btn-icon btn-success col" id=""
                                                        style="" style="margin-right:5px;">
                                                        <i class="mdi mdi-home"></i>
                                                        <?php echo get_phrase('active_theme'); ?>
                                                    </a>
                                                    <?php else : ?>
                                                    <a href="javascript::" class="btn btn-icon btn-secondary col-5"
                                                        id="" style="" style="margin-right:5px;"
                                                        onclick="activate_theme('<?php echo $installed_theme; ?>')">
...
function activate_theme(theme) {
    $.ajax({
        ...
        success: function(response) {
            if (response) {
                success_notify(theme.toUpperCase() +
                    ' <?php echo get_phrase('theme_successfully_activated') ?>');
                setTimeout(
                    function() {
                        location.reload();
                    }, 1000);
